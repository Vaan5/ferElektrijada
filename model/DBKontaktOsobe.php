...
class DBKontaktOsobe extends AbstractDBModel {
	...
	public function getAllForReport() {
		try {
			$pdo = $this->getPdo();
			$q = $pdo->prepare("SELECT *, sponzor.imeTvrtke sponzor, sponzor.adresaTvrtke adresa
									FROM kontaktosobe LEFT JOIN sponzor
									ON sponzor.idSponzora = kontaktosobe.idSponzora
									LEFT JOIN tvrtka ON tvrtka.idTvrtke = kontaktosobe.idTvrtke
									LEFT JOIN medij ON medij.idMedija = kontaktosobe.idMedija
									ORDER BY kontaktosobe.imeKontakt");
			$q->execute();
			return $q->fetchAll();
